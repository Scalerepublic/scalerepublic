...
const post = async (path: string, body: unknown, email: string) =>
    app.request(path, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json', ...(await authHeadersFor(app, email)) },
        body: JSON.stringify(body),
    })

const get = async (path: string, email: string) =>
    app.request(path, { headers: await authHeadersFor(app, email) })
...
describe('POST /api/v1/autotrade', () => {
    test('creates an ACTIVE stop-loss rule and returns 201', async () => {
        const { portfolioId, email } = await seedPortfolio({ cashBalance: '10000.00' })
    ...
    test('returns 400 when triggerDirection is missing', async () => {
        const { portfolioId, email } = await seedPortfolio()
        const { stockId } = await seedStock()
    ...
    test('returns 422 for a BUY the portfolio cannot afford', async () => {
        const { portfolioId, email } = await seedPortfolio({ cashBalance: '50.00' })
        const { stockId } = await seedStock()
...
describe('GET /api/v1/portfolio/:portfolioId/autotrades', () => {
    test('lists the rules for a portfolio', async () => {
        ...
        const created = await Promise.all([
            post('/api/v1/autotrade', { portfolioId, stockId: s1, ruleType: 'BUY', triggerDirection: 'AT_OR_BELOW', priceThreshold: 10, quantity: 1 }, email),
            post('/api/v1/autotrade', { portfolioId, stockId: s2, ruleType: 'SELL', triggerDirection: 'AT_OR_ABOVE', priceThreshold: 30, quantity: 2 }, email),
        ])
        const ids = await Promise.all(created.map(async (r) => ruleResponse.parse(await r.json()).data.id))

        const res = await get(`/api/v1/portfolio/${portfolioId}/autotrades`, email)
...
describe('POST /api/v1/autotrade/:ruleId/cancel', () => {
    test('cancels an active rule', async () => {
        ...
        const res = await post(`/api/v1/autotrade/${created.data.id}/cancel`, {}, email)
        expect(res.status).toBe(200)
        const { data } = ruleResponse.parse(await res.json())
    ...
    test('returns 404 when the rule is not active', async () => {
        ...
        await post(`/api/v1/autotrade/${created.data.id}/cancel`, {}, email)
    ...
    test('returns 404 for an unknown rule id', async () => {
        ...
        const res = await post('/api/v1/autotrade/does-not-exist/cancel', {}, email)
        expect(res.status).toBe(404)
        errorResponse.parse(await res.json())
    })
    ...
    test('the cancelled status is reflected in the list route', async () => {
        ...
        const { data } = rulesResponse.parse(
            await (await get(`/api/v1/portfolio/${portfolioId}/autotrades`, email)).json(),
        )
