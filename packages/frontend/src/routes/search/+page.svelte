...
<div class="mx-auto max-w-5xl px-4 py-8 md:px-8">
	...
	<div class="relative mb-8">
		...
		<input
			...
			class="h-11 w-full rounded-xl border border-input bg-card pl-10 pr-4 text-sm outline-none transition placeholder:text-muted-foreground/60 focus:border-accent focus:ring-2 focus:ring-accent/20"
		/>
	</div>
	...
	{#if showTrending}
		<div class="mb-5 flex items-center gap-3">
			<div class="flex items-center gap-2">
				...
				<div class="flex items-center gap-1.5 rounded-full border border-positive/20 bg-positive/10 px-2.5 py-1">
					...
					<span class="text-[10px] font-semibold uppercase tracking-widest text-positive">Live</span>
				</div>
			</div>
		...
		<div class="grid grid-cols-1 gap-4 min-[560px]:grid-cols-2 lg:grid-cols-3">
			{#each marketStore.trending as stock}
				<StockCard {stock} />
			{/each}
		</div>
		...
		<div class="grid grid-cols-1 gap-4 min-[560px]:grid-cols-2 lg:grid-cols-3">
			{#each marketStore.stocks as stock}
				<StockCard {stock} />
			{/each}
		</div>
	{:else}
		...
		{#if results.length === 0}
			<div class="flex flex-col items-center justify-center rounded-xl border border-dashed border-border py-20 text-center">
				...
				<p class="font-serif text-lg font-semibold text-muted-foreground">No results for "{query}"</p>
