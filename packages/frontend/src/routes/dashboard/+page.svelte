...
<div class="mx-auto max-w-5xl px-4 py-8 md:px-8">
	<div class="mb-8 flex flex-col gap-4 sm:flex-row sm:items-start sm:justify-between">
		...
		{#if userStore.profile.rank}
			<div
				class="flex w-full items-center justify-between gap-2 rounded-xl border border-accent/30 bg-accent/5 px-4 py-2.5 sm:w-auto sm:justify-start"
			>
				...
				<div class="text-right">
					...
					<p class="font-serif text-lg leading-none font-bold text-accent">
	...
	{#if portfolioStore.holdings.length === 0}
		...
		>
			...
			<NobleButton href="/search" class="mt-6 px-5">Browse Market</NobleButton>
		</div>
	{:else}
		<HoldingsTable holdings={portfolioStore.holdings} />

		<div
			class="mt-6 grid gap-4 rounded-xl border border-border bg-muted/30 px-5 py-4 sm:grid-cols-3"
		>
			<div>
				<p class="text-xs font-semibold tracking-widest text-muted-foreground uppercase">
					Portfolio Value
				</p>
				<p class="mt-1 font-serif text-xl font-bold text-primary">
			...
			<div class="sm:text-right">
				<p class="text-xs font-semibold tracking-widest text-muted-foreground uppercase">Total</p>
				<p class="mt-1 font-serif text-xl font-bold text-primary">
