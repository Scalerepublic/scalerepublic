...
<div class="page-shell">
	...
	{#if sectorBreakdown.length > 0}
		<div class="section-heading mt-10">
			<h2 class="text-xs font-semibold tracking-widest text-muted-foreground uppercase">Sector Allocation</h2>
			<span class="text-xs text-muted-foreground">{formatCurrency(portfolioStore.summary.holdingsValue)} invested</span>
		</div>

		<div class="mb-8 mt-4 grid gap-2.5">
			{#each sectorBreakdown as { sector, value, percent } (sector)}
				<div class="flex items-center gap-4">
					...
					<div class="flex w-32 shrink-0 items-center justify-end gap-2 text-right">
						...
						<span class="border border-border bg-muted px-1.5 py-0.5 font-mono text-xs text-muted-foreground">
							{percent.toFixed(1)}%
						</span>
					</div>
				</div>
	...
	{:else}
		...
		<div class="mt-6 grid gap-0 border border-border sm:grid-cols-3">
			<div class="border-b border-border px-5 py-4 sm:border-b-0 sm:border-r">
				<p class="text-[10px] font-semibold tracking-widest text-muted-foreground uppercase">Portfolio Value</p>
			...
			<div class="border-b border-border px-5 py-4 sm:border-b-0 sm:border-r sm:text-right">
				<p class="text-[10px] font-semibold tracking-widest text-muted-foreground uppercase">Cash Available</p>
				<p class="mt-1.5 font-serif text-xl font-bold text-primary">
